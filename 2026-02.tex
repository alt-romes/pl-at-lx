...
\begin{itemize}
    ...
    \item Step out
        \begin{itemize}
            ...
            \item Challenge: compiled code interleaved w/ interpreted code
        \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Inspecting variables and DebugView}
\begin{itemize}
    \item Decode any heap value as structured \texttt{Term}
    \item Find \texttt{DebugView} instances at runtime
    \item All custom built-in visualisations implemented with \texttt{DebugView}
        \begin{itemize}
            \item \texttt{"hello"} rather than \texttt{(:) 'h' (...)}
            \item \texttt{key => value} for Maps
        \end{itemize}
    \item \texttt{DebugView} instances can be provided by user
\end{itemize}
\end{frame}
...
\begin{frame}{Stack traces}
    \begin{itemize}
        ...
        \item Custom stack annotations
    \end{itemize}
\end{frame}

\begin{frame}{Exceptions}
    \begin{itemize}
        \item \texttt{raise\#} primop yields
        \item Inspect exception backtrace \texttt{ExceptionContext}
            \begin{itemize}
                \item Exception type and data
                \item Raise src location
                \item Backtraces
            \end{itemize}
    \end{itemize}
\end{frame}

\begin{frame}{Multi-threaded debugging (WIP)}
\begin{itemize}
    \item Display debuggee threads
    \item But not debugger threads!
    \item A stack per thread
    \item Pausing per thread
\end{itemize}
\end{frame}

\begin{frame}{External interpreter}
Same motivation as for GHC, even better for Debugger
\begin{itemize}
    \item Decouples the compiler from the interpreted code
        \begin{itemize}
            \item Debuggee can be profiled, while debugger not
        \end{itemize}
    \item Run debuggee in isolated process
        \begin{itemize}
            \item Threads no longer mixed
            \item Stdin/Stdout no longer mixed
        \end{itemize}
    \item Enables remote debugging
\end{itemize}
\end{frame}

\begin{frame}{Abstractions with external interpreter}
\begin{itemize}
    \item \texttt{ForeignRef} (typed ref that cannot be dereferenced locally)
    \item \texttt{TermParser} (parser combinator with \texttt{Term} rather than \texttt{String})
    \item \texttt{RemoteExpr} (eDSL for remote evaluation with batching and caching)
    \item \texttt{Program} (for querying the debugger from \texttt{DebugView})
\end{itemize}
\end{frame}
